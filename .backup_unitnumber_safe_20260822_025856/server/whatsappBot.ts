...
async function cmdTicketDetails(ticketId: string, user: BotUser): Promise<string> {
  ...
  return [
    ...
    `الفيلا: ${ticket.unitNumber} — ${ticket.clientName}`,
...
async function cmdVillaTickets(villa: string, user: BotUser): Promise<string> {
  ...
  const tickets = await prisma.ticket.findMany({
    where: { unitNumber: villa, ...projectWhere(projectIds), ...supervisorWhere(user) },
...
async function cmdSupervisorTickets(name: string, user: BotUser): Promise<string> {
  ...
  if (user.role === 'supervisor') {
    ...
    return `تذاكرك المفتوحة (${tickets.length}):\n` +
      tickets.map(t => `#${t.ticketId} — فيلا ${t.unitNumber} — ${statusLabel(t.status)}`).join('\n');
  }
...
async function prepareCloseTicket(
  ...
  return {
    ...
    execute: async () => {
      ...
      let report = buildClosureReport([{ ticketId: ticket.ticketId, unitNumber: ticket.unitNumber }], closureType, user.displayName);
...
async function prepareCloseVillaTickets(
  ...
  const tickets = await prisma.ticket.findMany({
    where: { unitNumber: villa, status: { notIn: ['closed', 'out_of_scope'] }, ...projectWhere(projectIds), ...supervisorWhere(user) },
  });
  ...
  return {
    ...
    execute: async () => {
      ...
      let report = buildClosureReport(tickets.map(t => ({ ticketId: t.ticketId, unitNumber: t.unitNumber })), closureType, user.displayName);
...
async function prepareCloseTicketsList(
  ...
  return {
    ...
    execute: async () => {
      ...
      let report = buildClosureReport(openTickets.map(t => ({ ticketId: t.ticketId, unitNumber: t.unitNumber })), closureType, user.displayName);
