...
// GET /api/contractors/suggest?unitId=X&specialtyKey=Y&projectId=Z
router.get("/suggest", requireAuth, async (req: AuthRequest, res) => {
  try {
    const { unitId, specialtyKey, projectId } = req.query;
    ...
    if (unitId) {
      const unit = await prisma.unit.findUnique({
        where: { id: String(unitId), ...(projectId ? { projectId: String(projectId) } : {}) }
      });
