...
router.post('/preview-appointment-range/:ticketId', requireAuth, async (req: AuthRequest, res) => {
  ...
  const { startDate, endDate, preferredTime, notes, clientName, unitId } = req.body;
  ...
  try {
    ...
    const msg = await buildAppointmentRangeMsg({
      ...
      unitNumber: firstTicket.unit?.unitNumber || '',
...
router.post('/appointment-range/:ticketId', requireAuth, async (req: AuthRequest, res) => {
  const uid = req.uid!;
  const { ticketId } = req.params;
  const { startDate, endDate, preferredTime, notes, phone, clientName, unitId } = req.body as {
    startDate: string;
    endDate: string;
  ...
  try {
    ...
    const msg = await buildAppointmentRangeMsg({
      ...
      ticketId: combinedTicketIds,
      unitNumber: (firstTicket as any).unit?.unitNumber || '',
      startDate: fmtDate(startDate),
      endDate: fmtDate(endDate),
      preferredTime,
      notes: notes || null,
