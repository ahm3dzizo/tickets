  ...
  try {
    ...
    const result = await prisma.$transaction(async (tx) => {
      ...
      for (const ticket of eligibleTickets) {
        ...
        if (existing) {
          // Already being worked on. Make sure ticket state is correct.
      ...
      // Mark appointment as completed only when all its tickets
      // are already completed. Otherwise keep it active.
