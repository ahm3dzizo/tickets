...
export function AssignContractorDialog({
  ...
  const unitNumber = tickets.length === 1 ? tickets[0].unitNumber : undefined;
  ...
  useEffect(() => {
    ...
    Promise.all([
      ...
      unitNumber
        ? contractorsApi.suggest({ projectId, unitNumber, blockNumber })
        : Promise.resolve([]),
    ])
      .then(([all, sugg]) => {
        setContractors(all);
        ...
        >
          ...
          <DialogHeader className="px-5 pt-5 pb-4 border-b border-border/50 shrink-0">
            ...
            <p className="text-sm text-muted-foreground mt-1">
              {tickets.length === 1
                ? `فيلا ${tickets[0].unitNumber} — ${tickets[0].clientName}`
