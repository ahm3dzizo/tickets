...
export function TicketForm({
  ...
  const handleVillaChange = (val: string) => {
    ...
    const match = formClients.find(
      c => String(c.unitNumber ?? '').trim() === val.trim()
    );

    if (match) {
      setClientId(match.id);
      setUnitId(match.unitId || '');
    } else {
      setClientId('');
      setUnitId('');
    }
  ...
  const handleSubmit = async (e: React.SyntheticEvent) => {
    ...
    if (!unitId) {
      toast.error('لا يمكن إنشاء التذكرة: العميل غير مرتبط بوحدة');
      return;
    }
    try {
      const currentClient = formClients.find(c => c.id === clientId);
      const proj = projects.find(p => p.id === projectId);
      const autoRefNumber = proj ? `${proj.abbreviation}-${unitNumber}` : unitNumber;
                ...
                <DropdownMenuTrigger
                  render={
                    ...
                    >
                      ...
                      <span>
                        {selectedClient
                          ? `${selectedClient.name} - ${selectedClient.unitNumber}`
