...
export function UnifiedAppointmentDialog({
  ...
  useEffect(() => {
    ...
    if (isEditMode && editGroup) {
      ...
      setCustomTime('09:00');
      setNotes('');
      setSelectedClientId(''); setSelectedUnitId(''); setSelectedUnit('');
  ...
  useEffect(() => {
    ...
    const unitId = isCalendarMode ? selectedUnitId : primaryTicket?.unitId;
      ...
      .then((res: any[]) => {
        const tks = res.filter(
          (t: any) => {
            ...
            const byUnit = !!t.unitId && String(t.unitId) === String(unitId);
            return (byClient || byUnit) && !['closed', 'out-of-scope', 'completed'].includes(t.status);
          }
        );
    ...
    appointmentsApi.getByUnit(pId, unitId)
      .then((res: any[]) => {
  ...
  // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [isCalendarMode, projectId, selectedUnitId, primaryTicket?.projectId, primaryTicket?.unitId]);
    ...
    const fetch = async () => {
      try {
        const result = await whatsappApi.previewAppointmentRange(multiIds, {
          ...
          unitNumber: primaryTicket?.unitNumber || '',
        });
      ...
      if (isEditMode) {
        ...
        if (hasExistingTickets && chosenTickets.length > 0) {
          ...
          if (existingApptId) {
            ...
            await appointmentsApi.create({
              projectId, unitId: selectedUnitId, unitNumber: selectedUnit,
  ...
  const dialogTitle = isEditMode
    ? `تعديل موعد — وحدة ${editGroup?.unitNumber}`
    : isCalendarMode
    ? 'إضافة موعد'
    : hasExistingAppt
    ? `تعديل موعد — وحدة ${primaryTicket!.unitNumber}`
    : canSendWhatsApp
        ...
        <div className="space-y-5 py-1">
            ...
            <div className="space-y-3 relative" ref={dropdownRef}>
              ...
              <div className="space-y-1.5">
                ...
                <div className="relative">
                  ...
                  <Input
                    placeholder="ابحث بالاسم، رقم الوحدة، أو اسم المشروع..."
                    value={isClientFocused ? clientSearch : (selectedClientId ? `${clientName} - وحدة ${selectedUnit}` : clientSearch)}
                    onChange={e => {
                  ...
                  {clients.length === 0
                    ...
                    : (() => {
                        ...
                        const filtered = clients.filter(c => {
                          ...
                          const unitMatch = c.unitNumber != null && String(c.unitNumber).toLowerCase().includes(s);
                          ...
                          return nameMatch || unitMatch || phoneMatch || projMatch;
                        }).slice(0, 50);
                        ...
                        return filtered.map(c => {
                          ...
                          return (
                            <div
                              key={`${c.id}-${c.projectId || ''}-${c.unitNumber || ''}`}
                              ...
                              onClick={() => {
                                ...
                                setSelectedUnitId(c.unitId || c.unit?.id || '');
                                setSelectedUnit(c.unitNumber);
          ...
          {hasConflict && (
            <div className="bg-red-500/10 border border-red-500/20 rounded-xl p-3 text-right">
              ...
              <p className="text-xs text-red-500/70">لا يمكن إنشاء موعد مكرر لنفس الوحدة في نفس اليوم.</p>
            </div>
          )}
