...
const ALL_COLUMNS: ColumnDef[] = [
  ...
  { key: 'unitNumber',     label: 'رقم الفيلا',      group: 'ticket',         groupLabel: 'معلومات التذكرة' },
...
const PRESETS: Record<string, { label: string; keys: string[] }> = {
  minimal:  {
    label: 'مختصر',
    keys:  ['projectAbbr', 'ticketId', 'unitNumber', 'clientName', 'issuedAt'],
  },
  standard: {
    label: 'أساسي',
    keys:  ['projectAbbr', 'ticketId', 'unitNumber', 'clientName', 'issuedAt', 'description', 'clientPhone'],
  },
  contractors: {
    label: 'مقاولين',
  ...
  extended: {
    label: 'موسع',
    keys:  ['projectAbbr', 'ticketId', 'unitNumber', 'clientName', 'issuedAt', 'description', 'clientPhone', 'status', 'priority', 'daysOpen', 'type', 'supervisors', 'contractorName', 'contractorNote'],
  },
  full: {
    label: 'كامل',
...
export function ExportTicketsModal({
  ...
  const getCellValue = (ticket: Ticket, colKey: string): string => {
    switch (colKey) {
      ...
      case 'unitNumber': return ticket.unitNumber  || '';
