...
export default function Appointments() {
  ...
  const [clientTicketsModal, setClientTicketsModal] = useState<{
    unitId: string;
    project: string;
  } | null>(null);
    ...
    for (const d of Object.keys(map)) {
      ...
      if (searchQuery) {
        ...
        groups = groups.filter(g => {
          ...
          return (
            (g.unitNumber && String(g.unitNumber).toLowerCase().includes(sq)) ||
  ...
  const downloadAllToCalendar = (e: React.MouseEvent) => {
    ...
    const events = groups.map(group => {
      ...
      return [
        ...
        `DTSTART:${dtStart}`,
        `DTEND:${dtEnd}`,
        `SUMMARY:${group.unitNumber}${types ? ' - ' + types : ''}`,
        desc ? `DESCRIPTION:${desc}` : '',
        `LOCATION:${group.unitNumber}`,
        `UID:unit-${group.unitId}-${dateOnly}@maintenance`,
        'END:VEVENT',
      ].filter(Boolean).join('\r\n');
    });
              ...
              >
                <div className="flex justify-between items-center border-b border-black/30 pb-1">
                  <h3 className="font-bold text-sm leading-tight">
                    {g.unitNumber} {projectsMap.get(g.projectId)?.name ? `• ${projectsMap.get(g.projectId).name}` : ''} <span className="font-normal text-xs text-gray-700">({g.clientPhone || tr.noPhone})</span>
                  </h3>
    ...
    <Layout>
      ...
      {addSpecOpen && addSpecData && (
        <QuickAddSpecialtyDialog
          ...
          unitNumber={addSpecData.unitNumber}
      ...
      {clientTicketsModal && (
        <ClientTicketsModal
          ...
          unitId={clientTicketsModal.unitId}
          ...
          <DialogContent className="max-w-md w-[95vw] rounded-2xl p-5" dir="rtl">
            <DialogHeader className="pb-3 border-b border-border">
              <DialogTitle className="flex items-center gap-2 text-base font-black">
                ...
                تعيين فني للفيلا {assignTechModal.unitNumber || ''}
              </DialogTitle>
            </DialogHeader>
