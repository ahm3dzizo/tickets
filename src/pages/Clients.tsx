...
export default function Clients() {
  ...
  const contactName = (c: any) => {
    ...
    return `${c.unitNumber || ''} ${first}`.trim();
  };
  ...
  const handleImportClients = async (data: any[]) => {
    ...
    const batch = data.map(item => {
      ...
      const unitNumber = String(item.unitNumber || item['رقم الفيلا'] || item['فيلا'] || (keys.length > 0 ? byIndex(0) : '')).trim();
      ...
      return clientsApi.create(importProjectId, {
        name, phone, unitNumber, blockNumber,
  ...
  const filtered = clients.filter(c => {
    ...
    if (search) {
      ...
      return (c.name || '').toLowerCase().includes(s) || String(c.unitNumber || '').includes(s) || String(c.phone || '').includes(s);
    }
    return true;
  });
  ...
  if (search) {
    const s = search.toLowerCase();
    filtered.sort((a, b) => {
      const aExact = String(a.unitNumber || '') === s || String(a.phone || '') === s || (a.name || '').toLowerCase() === s;
      const bExact = String(b.unitNumber || '') === s || String(b.phone || '') === s || (b.name || '').toLowerCase() === s;
      
      if (aExact && !bExact) return -1;
      if (!aExact && bExact) return 1;

      const aStarts = String(a.unitNumber || '').startsWith(s) || String(a.phone || '').startsWith(s) || (a.name || '').toLowerCase().startsWith(s);
      const bStarts = String(b.unitNumber || '').startsWith(s) || String(b.phone || '').startsWith(s) || (b.name || '').toLowerCase().startsWith(s);
                    ...
                    <div className="space-y-5 py-2">
                      ...
                      <div className={importProjectId ? '' : 'opacity-40 pointer-events-none select-none'}>
                        ...
                        <DataImport
                          ...
                          fieldDefs={[
                            { key: 'unitNumber', label: 'رقم الفيلا',   aliases: ['رقم الفيلا','الفيلا','فيلا','villa','A','__EMPTY'] },
                ...
                {loading ? (
                  ...
                  >
                      ...
                      <div className="flex items-center gap-2.5">
                        ...
                        <div className="text-right min-w-0">
                          ...
                          <div className="text-[11px] text-muted-foreground mt-0.5 sm:hidden">
                            {c.unitId
                              ? <a href={`/units/${c.unitId}`} onClick={e => e.stopPropagation()} className="text-primary hover:underline">{c.unitNumber}</a>
                              : c.unitNumber}
                    ...
                    <td className="px-3 py-2.5 hidden sm:table-cell">
                      {c.unitId
                        ? <a href={`/units/${c.unitId}`} onClick={e => e.stopPropagation()} className="text-sm font-bold text-primary hover:underline">{c.unitNumber}</a>
                        : <span className="text-sm font-bold text-foreground">{c.unitNumber}</span>}
                    </td>
          ...
          {selectedClient && (<>
            ...
            <div className="shrink-0 px-4 pt-4 pb-3 border-b border-border/60">
              ...
              <div className="flex items-center gap-3">
                ...
                <div className="flex-1 min-w-0">
                  ...
                  <div className="text-[11px] text-muted-foreground mt-0.5">فيلا {selectedClient.unitNumber}{selectedClient.blockNumber ? ` · بلوك ${selectedClient.blockNumber}` : ''}</div>
                </div>
            ...
            <div className="flex-1 overflow-y-auto px-4 py-3 space-y-3">
              ...
              <div className="grid grid-cols-2 gap-2">
                {[
                  { label: 'رقم الفيلا',  value: `فيلا ${selectedClient.unitNumber}` },
