...
function buildClosureReport(items: { ticketId: string; unitNumber: string }[], closureType: ClosureType, closedBy: string): string {
  const stamp = new Date().toISOString().slice(0, 16).replace('T', ' ');
  return [
    '📋 تقرير إغلاق',
    ...items.map(i => `#${i.ticketId} — فيلا ${i.unitNumber}`),
...
async function notifyClientOfClosure(
  ticket: {
    ...
    description: string; unitNumber: string; closureNotes?: string | null;
  ...
  try {
    ...
    const params = {
      ...
      description: ticket.description, unitNumber: ticket.unitNumber,
...
async function cmdClientPhone(villa: string, projectIds: string[] | null): Promise<string> {
  ...
  if (units.length === 0) {
    return `❌ مفيش فيلا رقم ${villa} في نطاق صلاحياتك.`;
  }
  ...
  return units.map(u => {
    ...
    if (!primary) {
      return `فيلا ${villa} (${u.project.name}): مفيش عميل مسجل.`;
    }
...
async function cmdTicketDetails(
  ticketId: string,
  user: BotUser,
): Promise<string> {
  ...
  if (!ticket) {
    return `❌ مفيش تذكرة رقم ${ticketId} في نطاق صلاحياتك.`;
  }

  const appointmentTime = ticket.appointment
    ? `${ticket.appointment.date}${ticket.appointment.time ? ' ' + ticket.appointment.time : ''}`
    : null;

  return [
    `🎫 تذكرة #${ticket.ticketId}`,
    `الفيلا: ${ticket.unit?.unitNumber || 'غير مرتبطة'} — ${ticket.client?.name || 'بدون عميل'}`,
...
async function cmdVillaTickets(
  villa: string,
  user: BotUser,
): Promise<string> {
  const projectIds = scopedProjectIds(user);
  ...
  if (tickets.length === 0) {
    return `مفيش تذاكر لفيلا ${villa} في نطاق صلاحياتك.`;
  }

  return `تذاكر فيلا ${villa} (${tickets.length}):\n` +
    tickets
      .map(t =>
        `#${t.ticketId} — ${statusLabel(t.status)} — ${t.description.slice(0, 40)}`
      )
      .join('\n');
}

async function cmdSupervisorTickets(
  name: string,
  user: BotUser,
): Promise<string> {
  const projectIds = scopedProjectIds(user);
  ...
  if (user.role === 'supervisor') {
    ...
    if (tickets.length === 0) {
      return `مفيهوش تذاكر مفتوحة في نطاقك.`;
    }

    return `تذاكرك المفتوحة (${tickets.length}):\n` +
      tickets
        .map(t =>
          `#${t.ticketId} — فيلا ${t.unit?.unitNumber || 'غير مرتبطة'} — ${statusLabel(t.status)}`
        )
        .join('\n');
  }
  ...
  if (tickets.length === 0) {
    return `المشرف ${supervisor.displayName} مفيهوش تذاكر مفتوحة في نطاق صلاحياتك.`;
  }
...
async function cmdReport(user: BotUser): Promise<string> {
  ...
  const [open, inProgress, pending, waiting, closed, total] =
    await Promise.all([
  ...
  const label = user.role === 'supervisor'
    ? 'تقريرك'
    : 'تقرير التذاكر';
...
async function prepareCloseTicket(
  ...
  if (ticket.status === 'closed' || ticket.status === 'out_of_scope') {
    return `تذكرة #${ticketId} مقفولة بالفعل.`;
  }
  ...
  return {
    ...
    execute: async () => {
      ...
      const notifyResult = await notifyClientOfClosure(
        ticketForNotify,
        closureType,
        user.uid,
      );

      let report = buildClosureReport(
        [{
...
async function prepareCloseVillaTickets(
  ...
  if (tickets.length === 0) {
    return `مفيش تذاكر مفتوحة لفيلا ${villa} في نطاق صلاحياتك.`;
  }
  ...
  return {
    ...
    execute: async () => {
      ...
      if (
        notifyResults.some(
          r => !r.sent && r.reason === 'not_connected'
        )
      ) {
...
async function prepareCloseTicketsList(
  ...
  if (tickets.length === 0) {
    return `❌ مفيش تذاكر بالأرقام دي في نطاق صلاحياتك.`;
  }

  const openTickets = tickets.filter(
    t => t.status !== 'closed' && t.status !== 'out_of_scope'
  );

  const alreadyClosed = tickets.filter(
    t => t.status === 'closed' || t.status === 'out_of_scope'
  );

  const notFound = ticketIds.filter(
    id => !tickets.some(t => t.ticketId === id)
  );

  if (openTickets.length === 0) {
    return `كل التذاكر دي مقفولة بالفعل.`;
  }
...
function finalizeAppointmentDraft(
  draft: PendingAppointmentDraft,
  user: { uid: string; role: string; displayName: string },
): { pending: PendingAction; summary: string } {
  const timeLabel = draft.time ? ` الساعة ${draft.time}` : '';
  const notesLabel = draft.notes ? ` — ملاحظات: ${draft.notes}` : '';

  return {
    summary: `هتحدد موعد للوحدة ${draft.unitNumber} (${draft.projectName}) يوم ${draft.date}${timeLabel}${notesLabel}. رد بـ "تأكيد" للتنفيذ.`,
    pending: {
      expiresAt: Date.now() + CONFIRM_TTL_MS,
      execute: async () => {
        ...
        return `✅ اتحدد موعد للوحدة ${draft.unitNumber} يوم ${draft.date}${timeLabel}.`;
      },
    },
  };
}
...
async function prepareScheduleAppointment(
  ...
  if (units.length === 0) {
    return `❌ مفيش وحدة رقم ${villa} في نطاق صلاحياتك.`;
  }

  if (units.length > 1) {
    return `فيه أكتر من وحدة بنفس الرقم (${units.map(u => u.project.name).join('، ')}), محتاج تحديد المشروع.`;
  }
  ...
  if (!date) {
    return `❌ مش فاهم اليوم "${dayText}". جرب: السبت، الأحد، اليوم، بكرة، بعد بكرة، أو تاريخ زي 2026-07-20.`;
  }
