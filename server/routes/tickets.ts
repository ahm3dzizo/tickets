...
router.post("/", requireAuth, async (req, res) => {
  ...
  try {
    ...
    // or change the Unit relation.
    // ============================================================

    const resolvedUnitId = asTrimmedString(data.unitId);

    if (!resolvedUnitId) {
      res.status(400).json({
        error: "يجب تحديد الوحدة لإنشاء التذكرة",
        code: "UNIT_REQUIRED",
      });
      return;
    }
    ...
    if (!unitCheck) {
      res.status(400).json({
        error: "الوحدة المحددة غير موجودة",
        code: "UNIT_NOT_FOUND",
      });
      return;
    }
    ...
    if (unitCheck.projectId !== projectId) {
      res.status(400).json({
        error: "الوحدة المحددة لا تتبع المشروع المحدد",
        code: "UNIT_PROJECT_MISMATCH",
      });
      return;
    }

    // If a client is supplied, it MUST be linked to this exact Unit.
    if (clientId) {
      ...
      if (!clientUnit) {
        res.status(400).json({
          error: "العميل المحدد غير مرتبط بالوحدة المحددة",
...
router.post("/bulk", requireAuth, async (req, res) => {
  ...
  try {
    ...
    // ============================================================
    // STRICT BULK PROJECT / UNIT / CLIENT VALIDATION
    // unitId is the ONLY source of the ticket unit.
    // unitNumber MUST NOT be used for resolving the Unit.
    // Every imported ticket MUST have:
    //   1) projectId
    //   2) unitId belonging to that project
    //   3) clientId linked to that exact unit
    // ============================================================

    const invalidRefs: any[] = [];

    const unitCache = new Map<string, { exists: boolean; projectId?: string }>();
    const clientUnitCache = new Map<string, boolean>();
    ...
    for (const t of normalized) {
      ...
      if (!t.projectId) {
        ...
        // Validate Unit existence + exact project ownership.
        let unitCheck = unitCache.get(t.unitId);
        ...
        if (!unitCheck) {
          ...
          unitCache.set(t.unitId, unitCheck);
        }

        if (!unitCheck.exists) {
          valid = false;
          reason = "UNIT_NOT_FOUND";
        } else if (unitCheck.projectId !== t.projectId) {
          valid = false;
          reason = "UNIT_PROJECT_MISMATCH";
        }
      }

      // Client MUST be linked to the EXACT Unit.
      if (valid && t.clientId) {
        const cacheKey = `${t.clientId}:${t.unitId}`;

        let clientLinked = clientUnitCache.get(cacheKey);

        if (clientLinked === undefined) {
          const clientUnit = await prisma.clientUnit.findFirst({
            where: {
              clientId: t.clientId,
              unitId: t.unitId,
          ...
          clientLinked = !!clientUnit;
          clientUnitCache.set(cacheKey, clientLinked);
        }
    ...
    if (invalidRefs.length > 0) {
      const sample = invalidRefs
        .slice(0, 10)
        .map(t => `${t.ticketId || `row-${t.index + 1}`}:${t.validationError}`)
        .join(", ");

      res.status(400).json({
        error: `هناك ${invalidRefs.length} تذاكر تحتوي على مراجع مشروع/وحدة/عميل غير صحيحة`,
        code: "BULK_REFERENCE_VALIDATION_FAILED",
        samples: sample,
      });
      return;
    }
...
router.put("/:id", requireAuth, async (req: AuthRequest, res) => {
  ...
  try {
    // ============================================================
    // STRICT UNIT / CLIENT VALIDATION
    // unitId is the ONLY source of the ticket unit.
    // Any unit/client change must preserve project consistency.
    // ============================================================

    const existingForValidation = await prisma.ticket.findUnique({
      where: { id: req.params.id },
      select: {
        projectId: true,
        unitId: true,
    ...
    if (!existingForValidation) {
      res.status(404).json({ error: "التذكرة غير موجودة", code: "TICKET_NOT_FOUND" });
      return;
    }
    ...
    const effectiveUnitId =
      data.unitId !== undefined
        ? asTrimmedString(data.unitId)
        : existingForValidation.unitId;

    const effectiveClientId =
      data.clientId !== undefined
        ? asTrimmedString(data.clientId)
        : existingForValidation.clientId;

    // Every ticket MUST have a Unit.
    if (!effectiveUnitId) {
      res.status(400).json({
        error: "لا يمكن أن تكون التذكرة بدون وحدة",
        code: "UNIT_REQUIRED",
      });
      return;
    }

    // Unit MUST exist and belong to the ticket project.
    const unitCheck = await prisma.unit.findUnique({
    ...
    if (unitCheck.projectId !== effectiveProjectId) {
      res.status(400).json({
        error: "الوحدة المحددة لا تتبع مشروع التذكرة",
        code: "UNIT_PROJECT_MISMATCH",
      });
      return;
    }

    // Client, when present, MUST be linked to this exact Unit.
...
router.post("/auto-link", requireAuth, async (req, res) => {
  try {
    ...
    // Can't auto-link without unitId — log and skip
    let linkedCount = 0;
    for (const ticket of unlinkedTickets) {
      // Tickets without unitId but we have no unitId;
